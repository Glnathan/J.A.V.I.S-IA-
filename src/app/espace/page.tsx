...
import { Loader2, Orbit, Satellite } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import { PLANETS, solarSystem } from "@/lib/solar";
...
const SPEEDS: { label: string; factor: number }[] = [
...
function SolarView() {
  ...
  const ref = useCanvas((ctx, w, h) => {
    ...
    ctx.font = "10px ui-monospace, monospace";
    ctx.fillText(
      `Éphémérides : ${new Date(now).toLocaleString("fr-FR")} — molette : zoom, glisser : déplacer`,
      14,
      h - 14,
    );
  });
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      offset.current += 100 * (SPEEDS[speedIdx].factor - 1);
    }, 100);
...
function SatellitesView() {
  ...
  const ref = useCanvas((ctx, w, h) => {
    ...
    ctx.fillStyle = `rgba(${r},${g},${b},0.7)`;
    ctx.font = "10px ui-monospace, monospace";
    ctx.fillText(`${positions.current.length ? `${drawn} satellites visibles sur ${positions.current.length}` : "chargement des éléments orbitaux…"} — glissez pour tourner le globe`, 14, h - 14);
  });
...
export default function EspacePage() {
  const [vue, setVue] = useState<"systeme" | "satellites">("systeme");
  ...
  return (
    <div className="hud-bg flex h-dvh flex-col text-slate-100">
      ...
      <footer className="relative px-5 pb-3 font-mono text-[10px] uppercase tracking-widest text-slate-500">
        Positions calculées en direct · éléments orbitaux Celestrak (cache 2 h) · {PLANETS.length} planètes
      </footer>
    </div>
  );
}
