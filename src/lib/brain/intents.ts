  ...
  // ─── Vision (Premium) ─────────────────────────────────────────────────
  {
    ...
    run: async (c) => {
      ...
      if (/(active|reactive|remets|verrouille)\s+(la\s+|le\s+)?(veille faciale|reconnaissance (du visage|faciale)|camera)\b/.test(f)) {
        ...
        return say(`Veille faciale réactivée, ${c.sir}. Je n'obéirai qu'en présence de votre visage.`, { actions: [R_SETTINGS] });
      }
      ...
      if (/(active|allume|ouvre|demarre|lance)\s+(la\s+|le\s+|ma\s+|mon\s+)?(vision|camera|webcam)/.test(f) || /^vision$/.test(f)) {
        ...
        return say(`Mode Vision activé. J'ouvre l'œil, ${c.sir}.`, { actions: [{ type: "vision", target: "panel" }, { type: "sound", name: "success" }] });
      }
